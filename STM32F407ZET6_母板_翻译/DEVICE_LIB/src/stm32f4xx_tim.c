 ...
 ===============================================================================
	...
	要使用定时器的输出比较模式，下面这些步骤是强制的：
      ...
      (#) Fill the TIM_OCInitStruct with the desired parameters including:
			填充TIM_OCInitStruct结构体期望的参数
        (++) The TIM Output Compare mode: TIM_OCMode
			TIM_OCMode代表定时器输出比较模式
      ...
      (#) Call TIM_OCxInit(TIMx, &TIM_OCInitStruct) to configure the desired 
          channel with the corresponding configuration
			执行TIM_OCxInit函数以相应的配置去设置期望的通道
       
      (#) Call the TIM_Cmd(ENABLE) function to enable the TIM counter.
			执行TIM_Cmd函数去使能定时器计数器
       
      -@- All other functions can be used separately to modify, if needed,
          a specific feature of the Timer. 
			这些所有的功能可以分开的去修改，如果需要，可以是定时器的一个指定特性。
      ...
      -@- If the corresponding interrupt or DMA request are needed, the user should:
			如果需要相应的中断或者DMA请求，用户应该：
        (+@) Enable the NVIC (or the DMA) to use the TIM interrupts (or DMA requests). 
			开启NVIC（或者DMA）去使用定时器中断（或者DMA请求）

...
u8 max30102_Bus_Write(I2C_Port_t *port, u8 Register_Address, u8 Word_Data)
{
	...
	IIC_Send_Byte(port, max30102_WR_address | I2C_WR);	/* 此处是写指令 */

	/* 第3步：发送ACK */
	if (IIC_Wait_Slave_Ack(port) != 0)
	{
		goto cmd_fail;	/* EEPROM器件无应答 */
	}

	/* 第4步：发送字节地址 */
	IIC_Send_Byte(port, Register_Address);
	if (IIC_Wait_Slave_Ack(port) != 0)
	{
		goto cmd_fail;	/* EEPROM器件无应答 */
	}
...
u8 max30102_Bus_Read(I2C_Port_t *port, u8 Register_Address)
{
	...
	/* 第4步：发送字节地址， */
	IIC_Send_Byte(port, (uint8_t)Register_Address);
	if (IIC_Wait_Slave_Ack(port) != 0)
	{
		goto cmd_fail;	/* EEPROM器件无应答 */
	}
	...
	IIC_Send_Byte(port, max30102_WR_address | I2C_RD);	/* 此处是读指令 */
	...
	/* 第9步：读取数据 */
	{
		data = IIC_Read_Byte(port, 1);	/* 读1个字节 */
...
void max30102_FIFO_ReadWords(I2C_Port_t *port, u8 Register_Address,u16 Word_Data[][2],u8 count)
{
	...
	while (no)
	{
		data1 = IIC_Read_Byte(port, 1);	
		data2 = IIC_Read_Byte(port, 1);
		...
		data2 = IIC_Read_Byte(port, 1);
		if(1==no)
			IIC_Send_Ack(port, 1);	/* 最后1个字节读完后，CPU产生NACK信号(驱动SDA = 1) */
		else
			IIC_Send_Ack(port, 0);
...
{	
	max30102_Bus_Read(port, REG_INTR_STATUS_1);
	max30102_Bus_Read(port, REG_INTR_STATUS_2);
	...
	/* 第9步：读取数据 */
	Data[0] = IIC_Read_Byte(port, 0);	
	Data[1] = IIC_Read_Byte(port, 0);	
	Data[2] = IIC_Read_Byte(port, 0);	
	Data[3] = IIC_Read_Byte(port, 0);
	Data[4] = IIC_Read_Byte(port, 0);	
	Data[5] = IIC_Read_Byte(port, 1);
...
//	max30102_Bus_Write(REG_LED2_PA, 0x47); 
	...
	max30102_Bus_Write(port, REG_INTR_ENABLE_1,0xc0);	// INTR setting
	max30102_Bus_Write(port, REG_INTR_ENABLE_2,0x00);
	max30102_Bus_Write(port, REG_FIFO_WR_PTR,0x00);  	//FIFO_WR_PTR[4:0]
	max30102_Bus_Write(port, REG_OVF_COUNTER,0x00);  	//OVF_COUNTER[4:0]
	max30102_Bus_Write(port, REG_FIFO_RD_PTR,0x00);  	//FIFO_RD_PTR[4:0]
	max30102_Bus_Write(port, REG_FIFO_CONFIG,0x0f);  	//sample avg = 1, fifo rollover=false, fifo almost full = 17
	max30102_Bus_Write(port, REG_MODE_CONFIG,0x03);  	//0x02 for Red only, 0x03 for SpO2 mode 0x07 multimode LED
	max30102_Bus_Write(port, REG_SPO2_CONFIG,0x27);  	// SPO2_ADC range = 4096nA, SPO2 sample rate (100 Hz), LED pulseWidth (400uS)  
	max30102_Bus_Write(port, REG_LED1_PA,0x24);   	//Choose value for ~ 7mA for LED1
	max30102_Bus_Write(port, REG_LED2_PA,0x24);   	// Choose value for ~ 7mA for LED2
	max30102_Bus_Write(port, REG_PILOT_PA,0x7f);   	// Choose value for ~ 25mA for Pilot LED
...
{
	max30102_Bus_Write(port, REG_MODE_CONFIG,0x40);
	max30102_Bus_Write(port, REG_MODE_CONFIG,0x40);
}
...
//  IIC_WriteBytes(I2C_WRITE_ADDR, ach_i2c_data, 2);
	IIC_Write_One_Byte(port, I2C_WRITE_ADDR,uch_addr,uch_data);
}
...
//   *puch_data=(uint8_t) ch_i2c_data;
	IIC_Read_One_Byte(port, I2C_WRITE_ADDR,uch_addr,puch_data);
}
...
void maxim_max30102_read_fifo(I2C_Port_t *port, uint32_t *pun_red_led, uint32_t *pun_ir_led)
{
  ...
  maxim_max30102_read_reg(port, REG_INTR_STATUS_1, &uch_temp);
  maxim_max30102_read_reg(port, REG_INTR_STATUS_2, &uch_temp);
  
  IIC_Read_Bytes(port, I2C_WRITE_ADDR,REG_FIFO_DATA,(u8 *)ach_i2c_data,6);
